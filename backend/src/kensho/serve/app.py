...
# backend/src/kensho/serve/app.py -> backend -> repo root -> frontend/dist
#
# `frontend/dist` is the built React console, and it is committed to the repository
...
# complete instruction rather than step three of five. `cd frontend && npm run build`
# regenerates it from `frontend/src`.
WEB_DIST = Path(__file__).resolve().parents[4] / "frontend" / "dist"
...
def create_app(config: ServerConfig | None = None) -> FastAPI:
    ...
    def console() -> FileResponse | JSONResponse:
        if not WEB_INDEX.exists():
            return JSONResponse(
                {"detail": f"Console not built at {WEB_INDEX}. Run `cd frontend && npm install "
